...
func (r *Repository) FindByUsername(ctx context.Context, username string) (*domain.Account, error) {
	query := `
		SELECT a.id_account, a.username, a.password, a.id_role, a.created_at, a.updated_at,
	...
	err := r.db.QueryRow(ctx, query, username).Scan(
		&account.IDAccount, &account.Username, &account.Password, &account.IDRole, &account.CreatedAt, &account.UpdatedAt,
...
func (r *Repository) StoreAccount(ctx context.Context, account *domain.Account) error {
	query := `
		INSERT INTO auth.accounts (username, password, id_role, farm_id)
		VALUES ($1, $2, $3, $4)
		RETURNING id_account, created_at, updated_at`

	return r.db.QueryRow(ctx, query, account.Username, account.Password, account.IDRole, account.FarmID).Scan(
