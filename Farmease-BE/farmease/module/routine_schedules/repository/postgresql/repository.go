...
func (r *routineScheduleRepository) Store(ctx context.Context, rs *domain.RoutineSchedule) error {
	...
	`,
		rs.Title, rs.Description, rs.Category, rs.Frequency, rs.DaysOfWeek, rs.DayOfMonth, rs.StartDate, rs.EndDate, st, et, rs.Priority, rs.IDCage, rs.IDAccount, rs.Rincian, rs.IsActive,
...
func (r *routineScheduleRepository) Update(ctx context.Context, rs *domain.RoutineSchedule) error {
	...
	_, err := r.db.Exec(ctx, `
		...
		WHERE id = $16
	`,
		rs.Title, rs.Description, rs.Category, rs.Frequency, rs.DaysOfWeek, rs.DayOfMonth, rs.StartDate, rs.EndDate, st, et, rs.Priority, rs.IDCage, rs.IDAccount, rs.Rincian, rs.IsActive, rs.ID,
	)
