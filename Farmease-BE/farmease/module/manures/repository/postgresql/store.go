...
func (r *Repository) Store(ctx context.Context, m *domain.Manure) error {
	...
	if m.IDCage != "" {
		...
		err := r.db.QueryRow(ctx, `SELECT id_cage FROM master.cages WHERE cage_code = $1 OR id_cage::text = $1 LIMIT 1`, m.IDCage).Scan(&resolvedCageID)
	...
	query := `INSERT INTO logistics.manures (id_sheep, id_cage, activity_type, amount, unit, external_destination_id, destination_type, notes) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id_manure, created_at`
