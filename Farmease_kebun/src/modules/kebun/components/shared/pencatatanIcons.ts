...
export function getJenisIcon(jenis: string): string {
  ...
  if (lower.includes('stok pakan') || lower.includes('stok pupuk') || lower.includes('stok')) return '/icon/document.png'
...
export function getFormIcon(kindTitle: string): string {
  switch (kindTitle) {
    ...
    case 'Stok Pakan':   return '/icon/document.png'
    case 'Stok Pupuk':   return '/icon/document.png'
