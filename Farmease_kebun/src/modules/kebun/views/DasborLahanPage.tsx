...
export default defineComponent({
  ...
  setup() {
    ...
    // Tree Phases count for the bar chart
    const phaseData = computed(() => {
      ...
      let pembibitan = 0
      let panen = 0
      let tidakProduktif = 0

      landCrops.forEach(c => {
        const type = (c.type || '').toLowerCase()
        if (type.includes('generatif')) generatif++
        else if (type.includes('vegetatif')) vegetatif++
        else if (type.includes('pembibitan') || type.includes('bibit')) pembibitan++
        else if (type.includes('panen')) panen++
        else if (type.includes('tidak produktif') || type.includes('belum') || type.includes('juvenile')) tidakProduktif++
        else panen++
      })

      return { generatif, vegetatif, pembibitan, panen, tidakProduktif }
    })
        ...
        <div class="detail-shell" style="max-width: 1200px; margin: 0 auto; width: 100%;">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
            ...
            <div style={{ background: '#FFF', border: '1.5px solid #E6D9CE', borderRadius: '12px', padding: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              {/* Legend Grid */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: '0.5rem 1rem', marginBottom: '1.25rem', fontSize: '0.73rem', fontWeight: '700', color: '#374151', width: '100%', justifyContent: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', justifyContent: 'center' }}>
                  ...
                  <span>Pembibitan: {phaseData.value.pembibitan}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', justifyContent: 'center' }}>
                  <span style={{ width: '10px', height: '10px', backgroundColor: '#A5B892', borderRadius: '2px', display: 'inline-block' }}></span>
                  <span>Panen: {phaseData.value.panen}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', justifyContent: 'center' }}>
                  <span style={{ width: '10px', height: '10px', backgroundColor: '#3C3026', borderRadius: '2px', display: 'inline-block' }}></span>
                  <span>Tidak Prod: {phaseData.value.tidakProduktif}</span>
                </div>
              </div>
              ...
              <div style="position: relative; width: 100%; height: 160px;">
                <svg viewBox="0 0 400 160" width="100%" height="100%" style="overflow: visible;">
                  ...
                  {/* Y Axis Labels */}
                  {(() => {
                    const maxVal = Math.max(5, phaseData.value.generatif, phaseData.value.vegetatif, phaseData.value.pembibitan, phaseData.value.panen, phaseData.value.tidakProduktif)
                    return (
                      <>
                        ...
                        {/* Bars & Labels */}
                        {/* Generatif Bar */}
                        <rect x="45" y={130 - (phaseData.value.generatif / maxVal) * 100} width="20" height={(phaseData.value.generatif / maxVal) * 100} fill="#7c8e65" rx="3" />
                        <text x="55" y="145" font-size="7" fill="#9ca3af" text-anchor="middle">Gen</text>

                        {/* Vegetatif Bar */}
                        <rect x="115" y={130 - (phaseData.value.vegetatif / maxVal) * 100} width="20" height={(phaseData.value.vegetatif / maxVal) * 100} fill="#233512" rx="3" />
                        <text x="125" y="145" font-size="7" fill="#9ca3af" text-anchor="middle">Veg</text>

                        {/* Pembibitan Bar */}
                        <rect x="185" y={130 - (phaseData.value.pembibitan / maxVal) * 100} width="20" height={(phaseData.value.pembibitan / maxVal) * 100} fill="#d2c5b3" rx="3" />
                        <text x="195" y="145" font-size="7" fill="#9ca3af" text-anchor="middle">Bibit</text>

                        {/* Panen Bar */}
                        <rect x="255" y={130 - (phaseData.value.panen / maxVal) * 100} width="20" height={(phaseData.value.panen / maxVal) * 100} fill="#a5b892" rx="3" />
                        <text x="265" y="145" font-size="7" fill="#9ca3af" text-anchor="middle">Panen</text>

                        {/* Tidak Produktif Bar */}
                        <rect x="325" y={130 - (phaseData.value.tidakProduktif / maxVal) * 100} width="20" height={(phaseData.value.tidakProduktif / maxVal) * 100} fill="#372d24" rx="3" />
                        <text x="335" y="145" font-size="7" fill="#9ca3af" text-anchor="middle">Tidak Prod</text>
                      </>
                    )
                  })()}
