...
import { sheep, loading, error, fetchSheep, addSheep, sheepStats, updateSheepStatus, mutationHistory, SHEEP_STATUS_OPTIONS } from '@/store/livestock';
...
export default defineComponent({
  ...
  setup() {
    ...
    const statusModalId = ref<string | null>(null);
    ...
    const handleAddDomba = async () => {
      try {
        ...
        await addSheep({
          ...
          id_type: newDomba.value.type,
          gender: newDomba.value.gender === 'Jantan' ? 'jantan' : 'betina',
          date_of_birth: newDomba.value.birth_date,
    ...
    const handleUpdateStatus = async () => {
      if (!statusModalId.value || !newStatusValue.value) return;
      try {
        await updateSheepStatus(statusModalId.value, newStatusValue.value);
        statusModalId.value = null;
    ...
    const filtered = computed(() => {
      ...
      return cageFilteredSheep.filter(sheepItem => {
        const query = search.value.toLowerCase();
        const matchSearch = !query || sheepItem.name.toLowerCase().includes(query) || sheepItem.code.toLowerCase().includes(query) || sheepItem.type.toLowerCase().includes(query);
    ...
    return () => (
      <div class="animate-fade-in-up">
        ...
        <div class="bg-white rounded-5 border shadow-sm p-4 p-md-5 mb-4">
          ...
          <div class="mb-4">
            ...
            <div class="d-flex gap-2 mb-3">
              {(['aktif', 'mutasi'] as const).map(tab => (
                ...
                >
                  {tab === 'aktif' ? `Aktif (${sheep.value.filter(s => s.cage_code === activeCageCode.value && !['Mati','Terjual','Disembelih'].includes(s.status)).length})` : `Riwayat Keluar (${mutationHistory.value.length})`}
                </button>
              ))}
            </div>
          ...
          <div class="row g-3">
            {loading.value ? (
              ...
              mutationHistory.value.length === 0 ? (
                <div class="col-12 text-center py-5 text-secondary">
                  <p>Belum ada riwayat mutasi keluar.</p>
                </div>
              ) : (
                mutationHistory.value.map((sheepItem) => (
                  <div class="col-12 col-md-6 col-xl-4" key={sheepItem.id}>
                    <div class="peternakan-item-card h-100 flex-column align-items-stretch" style={{ opacity: 0.75 }}>
                      ...
                      <div class="d-flex justify-content-end mt-3">
                        <button class="peternakan-action-btn" onClick={() => selectedTernakId.value = sheepItem.id}>Detail</button>
                      </div>
                    </div>
                  </div>
                ))
                  ...
                  <div class="peternakan-item-card h-100 flex-column align-items-stretch">
                    ...
                    <div class="d-flex align-items-center justify-content-end gap-2 mt-3 pt-3 border-top">
                      {activeTab.value === 'aktif' && (
                        ...
                        onClick={() => { statusModalId.value = sheepItem.id; newStatusValue.value = sheepItem.status; }}
                        >
                          Ubah Status
                        </button>
                      )}
          ...
          <div class="peternakan-modal-overlay" onClick={() => isAddModalOpen.value = false}>
            <div class="peternakan-modal-card animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
              ...
              <div class="peternakan-modal-body">
                <div class="row g-3">
                  <div class="col-6">
                    <label class="pencatatan-label">Kode Domba (Ear Tag)</label>
                    <CustomInput 
                  ...
                  <div class="col-6">
                    <label class="pencatatan-label">Ras/Jenis</label>
                    <CustomSelect 
                  ...
                  <div class="col-12">
                    <label class="pencatatan-label">Asal Ternak</label>
                    <CustomSelect
        ...
        {/* Modal Ubah Status Domba */}
        {statusModalId.value && (
          <div class="peternakan-modal-overlay" onClick={() => statusModalId.value = null}>
            <div class="peternakan-modal-card animate-fade-in-up" style={{ maxWidth: '360px' }} onClick={(e) => e.stopPropagation()}>
              <div class="peternakan-modal-header">
                <button class="peternakan-modal-close" onClick={() => statusModalId.value = null}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                    <line x1="18" y1="6" x2="6" y2="18"></line>
                    <line x1="6" y1="6" x2="18" y2="18"></line>
                  </svg>
              ...
              <div class="peternakan-modal-body">
                <label class="pencatatan-label">Status Baru</label>
                <CustomSelect
                ...
                />
                <div class="mt-4 pt-3 border-top border-light d-flex gap-3">
                  <button class="btn btn-light grow fw-bold py-2 rounded-pill" onClick={() => statusModalId.value = null}>Batal</button>
                  <button class="peternakan-primary-btn grow m-0 justify-content-center" onClick={handleUpdateStatus}>Simpan</button>
                </div>
              </div>
            </div>
          </div>
